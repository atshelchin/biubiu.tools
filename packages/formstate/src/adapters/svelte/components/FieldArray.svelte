<script lang="ts">
	...
	function insert(index: number, value: unknown) {
		...
		// P0: 使用公开 API 重新映射字段状态
		formState.remapArrayFields(name, 'insert', index);
	}
	...
	function remove(index: number) {
		...
		// P0: 使用公开 API 重新映射字段状态
		formState.remapArrayFields(name, 'remove', index);
	}
	...
	function move(from: number, to: number) {
		...
		// P0: 使用公开 API 重新映射字段状态
		formState.remapArrayFields(name, 'move', from, to);
	}
