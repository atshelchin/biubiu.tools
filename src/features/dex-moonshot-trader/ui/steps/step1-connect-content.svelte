<script lang="ts">
	...
	$effect(() => {
		if (connectStore.isConnected && connectStore.address && selectedChainId) {
			appState.connectedWallet = connectStore.address;
			appState.selectedChainId = selectedChainId;
		}
	});
...
<div class="step-content">
	<StepContentHeader
		...
		description="Connect your wallet and choose the blockchain network for trading"
	/>
		...
		{#if isReadyToContinue}
			<div class="continue-section">
				<button class="continue-button" onclick={handleContinue}>
					<span>Continue to Token Input</span>
					<svg
						width="20"
						height="20"
						viewBox="0 0 24 24"
				...
				<p class="continue-hint">
					Your wallet is connected and ready. Click to enter token address.
				</p>
			</div>
		{/if}
	{/if}
