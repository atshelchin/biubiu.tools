...
<div class="action-buttons">
	<button class="btn-secondary" onclick={onEstimate} disabled={isEstimating}>
		{#if isEstimating}
			...
			{i18n.t('routes/apps/wallet-sweep.step5.content.actions.estimating')}
		{:else}
			{i18n.t('routes/apps/wallet-sweep.step5.content.actions.estimate_cost')}
		{/if}
	</button>
	...
	{#if showEstimate}
		<button class="btn-execute" onclick={onExecute} disabled={!canExecute || isExecuting}>
			{#if isExecuting}
				<Loader2 size={18} class="spinning" />
				{i18n.t('routes/apps/wallet-sweep.step5.content.actions.executing')}
			{:else}
				{i18n.t('routes/apps/wallet-sweep.step5.content.actions.execute_sweep')}
			{/if}
		</button>
	{/if}
</div>
