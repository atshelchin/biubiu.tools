<script lang="ts">
	import type { TemporaryWallet } from '@/features/wallet-sweep/types/fee';
	import {
	...
	async function handleSendGasFromWallet() {
		if (!connectStore.address || !temporaryWallet) {
			errorMessage = i18n.t('routes/apps/wallet-sweep.temporary_wallet.error_no_wallet');
			return;
		}

		if (!gasSendAmount || Number(gasSendAmount) <= 0) {
			errorMessage = i18n.t('routes/apps/wallet-sweep.temporary_wallet.error_invalid_amount');
			return;
		}
		...
		try {
			...
			successMessage = i18n.t('routes/apps/wallet-sweep.temporary_wallet.success_transfer', {
				amount,
...
<div class="temporary-wallet-manager">
	<div class="manager-header">
		<Key size={20} />
		<h4>{i18n.t('routes/apps/wallet-sweep.temporary_wallet.title')}</h4>
	</div>
	...
	{#if !temporaryWallet}
		...
		<div class="create-section">
			<p class="create-description">
				{i18n.t('routes/apps/wallet-sweep.temporary_wallet.description')}
			</p>

			<div class="create-warning">
				<AlertCircle size={18} />
				<div>
					<strong>{i18n.t('routes/apps/wallet-sweep.temporary_wallet.important')}:</strong>
					{i18n.t('routes/apps/wallet-sweep.temporary_wallet.warning_text')}
					<ul>
						<li>{i18n.t('routes/apps/wallet-sweep.temporary_wallet.step_download')}</li>
						<li>
							{i18n.t('routes/apps/wallet-sweep.temporary_wallet.step_fund', {
								amount: formattedGasCost,
								symbol: networkSymbol
							})}
						</li>
						<li>{i18n.t('routes/apps/wallet-sweep.temporary_wallet.step_clear')}</li>
					</ul>
				</div>
			</div>
			...
			<button class="btn-create" onclick={handleCreateTemporaryWallet} disabled={isCreating}>
				{#if isCreating}
					...
					{i18n.t('routes/apps/wallet-sweep.temporary_wallet.creating')}
				{:else}
					<Key size={20} />
					{i18n.t('routes/apps/wallet-sweep.temporary_wallet.btn_create')}
				{/if}
			</button>
		...
		<div class="wallet-display">
			<!-- <div class="wallet-status">
				<div class="status-badge">
					<Check size={16} />
					<span>{i18n.t('routes/apps/wallet-sweep.temporary_wallet.wallet_ready')}</span>
				</div>
				<p class="status-info">
					{i18n.t('routes/apps/wallet-sweep.temporary_wallet.created_at', {
			...
			<div class="wallet-field">
				<div class="field-label">{i18n.t('routes/apps/wallet-sweep.temporary_wallet.address')}</div>
				<div class="field-content">
					<code class="field-value">{temporaryWallet.address}</code>
					<button
						class="btn-icon"
						onclick={() => handleCopyToClipboard(temporaryWallet?.address || '', 'address')}
						title={i18n.t('routes/apps/wallet-sweep.temporary_wallet.copy_address')}
					>
					...
					<button
						class="btn-qr"
						onclick={toggleQR}
						title={i18n.t('routes/apps/wallet-sweep.temporary_wallet.show_qr')}
					>
						<QrCode size={16} />
					</button>
				</div>
			...
			<!-- Balance Display -->
			<div class="wallet-field">
				<div class="field-label">{i18n.t('routes/apps/wallet-sweep.temporary_wallet.balance')}</div>
				<div class="field-content balance-content">
					{#if isLoadingBalance}
						<Loader2 size={16} class="spinning" />
						<span class="balance-text"
							>{i18n.t('routes/apps/wallet-sweep.temporary_wallet.loading')}</span
						>
					{:else}
						<Wallet size={16} />
					...
					<button
						...
						title={i18n.t('routes/apps/wallet-sweep.temporary_wallet.refresh_balance')}
					>
						<RefreshCw size={16} />
					</button>
				</div>
			...
			{#if showQR}
				<div class="qr-display">
					...
					<p class="qr-hint">{i18n.t('routes/apps/wallet-sweep.temporary_wallet.qr_hint')}</p>
				</div>
			{/if}
			...
			<div class="wallet-actions">
				<button class="btn-download" onclick={handleDownloadWallet}>
					<Download size={16} />
					{i18n.t('routes/apps/wallet-sweep.temporary_wallet.btn_download')}
				</button>
				<button class="btn-fund-gas" onclick={openGasFundingModal}>
					<Send size={16} />
					{i18n.t('routes/apps/wallet-sweep.temporary_wallet.fund_title')}
				</button>
			</div>
