...
{#if showEstimate && estimateData}
	<div class="estimate-card" transition:slide>
		<h4>{i18n.t('wallet-sweep.step5.content.estimate.title')}</h4>
		<div class="estimate-row">
			<span>{i18n.t('wallet-sweep.step5.content.estimate.total_transactions')}</span>
			<strong>{estimateData.totalTransactions}</strong>
		</div>
		<div class="estimate-row">
			<span>{i18n.t('wallet-sweep.step5.content.estimate.estimated_gas')}</span>
			<strong
				>{i18n.t('wallet-sweep.step5.content.estimate.gas_units', {
					amount: estimateData.estimatedGas.toString()
				})}</strong
			>
		</div>
		<div class="estimate-row">
			<span>{i18n.t('wallet-sweep.step5.content.estimate.estimated_cost')}</span>
			<strong
				>{i18n.t('wallet-sweep.step5.content.estimate.cost_eth', {
