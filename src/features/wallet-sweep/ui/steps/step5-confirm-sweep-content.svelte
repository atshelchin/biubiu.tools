<script lang="ts">
	...
	async function handleResumeAfterGasRefill() {
		...
		if (!canResume) {
			...
			alert(i18n.t('tools.wallet_sweep.step5.content.errors.still_insufficient_gas'));
			return;
		}
	...
	async function handleExecuteSweep() {
		...
		if (!currentNetwork || currentNetwork.rpcEndpoints.length === 0) {
			alert(i18n.t('tools.wallet_sweep.step5.content.errors.no_rpc'));
			return;
		}
...
<StepContent>
	...
	<!-- Target Address -->
	<TargetAddressInfo
		label={i18n.t('tools.wallet_sweep.step5.target_address_label')}
		address={targetAddress}
		hint={i18n.t('tools.wallet_sweep.step5.target_address_hint')}
	/>
	...
	{#if gasMonitor.isPausedForGas}
		<GasWarningBanner
			title={i18n.t('tools.wallet_sweep.step5.content.gas_insufficient_title')}
			message={i18n.t('tools.wallet_sweep.step5.content.gas_insufficient_message', {
			...
			buttonText={i18n.t('tools.wallet_sweep.step5.content.check_and_resume')}
