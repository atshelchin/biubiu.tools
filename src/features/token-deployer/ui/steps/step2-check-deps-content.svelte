<script lang="ts">
	...
	let summaryTitle = $derived(
		summary?.allPassed
			? i18n.t('tools.token_deployer.step2.content.all_dependencies_satisfied')
			: i18n.t('tools.token_deployer.step2.content.dependency_issues_found')
	);
	let summaryMessage = $derived(
		summary?.allPassed
			? i18n.t('tools.token_deployer.step2.content.network_properly_configured')
			: i18n.t('tools.token_deployer.step2.content.resolve_issues_before_continuing')
	);
