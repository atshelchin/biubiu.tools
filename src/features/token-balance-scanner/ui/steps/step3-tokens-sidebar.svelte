...
<StepSidebar title="" description="" stepNumber={3} showWalletConnect={true}>
	{#if selectedCount > 0}
		<div transition:fade>
			<StepSummary title={i18n.t('token-balance-scanner.step3.sidebar.selected_tokens_title')}>
				<div class="summary-item">
					<span>{i18n.t('token-balance-scanner.step3.sidebar.count_label')}</span>
					<strong>{selectedCount}</strong>
				</div>
			</StepSummary>
		</div>
	{:else}
		<EmptyHint message={i18n.t('token-balance-scanner.step3.sidebar.empty_hint')} />
	{/if}
</StepSidebar>
