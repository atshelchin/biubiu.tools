<script lang="ts">
	...
	async function restoreSession(sessionId: string) {
		try {
			...
			if (!session) {
				step5State.setError(
					i18n.t('routes/apps/token-balance-scanner.session.error.not_found') || 'Session not found'
				);
				return;
			}
			...
			step5State.setError(
				i18n.t('routes/apps/token-balance-scanner.session.error.restore_failed') ||
	...
	async function handleStartScan() {
		if (!currentNetwork || !connectStore.currentChainId) {
			step5State.setError(
				i18n.t('routes/apps/token-balance-scanner.step5.error.no_network') || 'No network selected'
			);
			return;
		}
		...
		if (tokens.length === 0) {
			step5State.setError(
				i18n.t('routes/apps/token-balance-scanner.step5.error.no_tokens') || 'No tokens selected'
			);
			return;
		}
		...
		if (step4State.wallets.length === 0) {
			step5State.setError(
				i18n.t('routes/apps/token-balance-scanner.step5.error.no_wallets') || 'No wallets added'
			);
			return;
		}
...
<StepContent>
	<StepContentHeader
		title={i18n.t('routes/apps/token-balance-scanner.step5.title') || 'Scan & View Results'}
		description={i18n.t('routes/apps/token-balance-scanner.step5.description') ||
