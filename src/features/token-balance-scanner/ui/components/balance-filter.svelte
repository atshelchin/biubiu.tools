<script lang="ts">
	...
	const balanceStatusOptions = $derived([
		{
			value: 'all' as const,
			label: i18n.t('tools.token_balance_scanner.step5.filter.status_all') || 'All'
		},
		{
...
<div class="balance-filter">
	<div class="filter-header">
		<div class="filter-title">
			<Filter size={16} />
			<span>{i18n.t('tools.token_balance_scanner.step5.filter.title') || 'Filter'}</span>
		</div>
		{#if isFilterActive}
			<button class="reset-btn" onclick={onReset}>
				<RotateCcw size={14} />
				<span>{i18n.t('tools.token_balance_scanner.step5.filter.reset') || 'Reset'}</span>
			</button>
		{/if}
	</div>
	...
	<div class="filter-result">
		{#if isFilterActive}
			<span class="result-count">
				{i18n.t('tools.token_balance_scanner.step5.filter.showing', {
		...
		{:else}
			<span class="result-count">
				{i18n.t('tools.token_balance_scanner.step5.filter.total', { count: totalCount }) ||
					`${totalCount} results`}
			</span>
		{/if}
	</div>
