<script lang="ts">
	...
	const columns = $derived.by(() => {
		const cols: DataTableColumn<AddressBalance>[] = [
			{
				id: 'address',
				header: i18n.t('routes/apps/token-balance-scanner.step5.balances.address') || 'Address',
				sortable: true,
				minWidth: '160px',
...
<div class="balance-table-container">
		...
		<div class="filter-row">
			...
			{#if isFilterActive}
				<button class="reset-btn" onclick={onFilterReset}>
					{i18n.t('routes/apps/token-balance-scanner.step5.filter.reset') || 'Reset'}
				</button>
			{/if}
		</div>
	</div>
	...
	<DataTable
		...
		{getRowClass}
		{filteredCount}
		resultsLabel={i18n.t('routes/apps/token-balance-scanner.step5.balances.results') || 'results'}
		emptyMessage={i18n.t('routes/apps/token-balance-scanner.step5.balances.no_results') ||
			'No results'}
	>
