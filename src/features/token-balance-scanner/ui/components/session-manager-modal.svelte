<script lang="ts">
	...
	function getStatusText(status: SessionStatus): string {
		switch (status) {
			case 'scanning':
				return i18n.t('routes/apps/token-balance-scanner.session.status.scanning') || 'Scanning';
			case 'paused':
				return i18n.t('routes/apps/token-balance-scanner.session.status.paused') || 'Paused';
			case 'completed':
				return i18n.t('routes/apps/token-balance-scanner.session.status.completed') || 'Completed';
			case 'abandoned':
				return i18n.t('routes/apps/token-balance-scanner.session.status.abandoned') || 'Abandoned';
			default:
				return status;
		}
	}
	...
	async function handleAbandon(session: ScanSession) {
		...
		try {
			...
			deleteError =
				i18n.t('routes/apps/token-balance-scanner.session.error.abandon_failed') ||
	...
	async function handleDelete(session: ScanSession) {
		...
		try {
			...
			deleteError =
				i18n.t('routes/apps/token-balance-scanner.session.error.delete_failed') ||
...
<div class="modal-overlay" onclick={onClose}>
	...
	>
		<div class="modal-header">
			<h2>{i18n.t('routes/apps/token-balance-scanner.session.title') || 'Scan Sessions'}</h2>
		...
		<div class="modal-body">
			...
			{#if localSessions.length === 0}
				<div class="empty-state">
					<Clock size={48} />
					<p>{i18n.t('routes/apps/token-balance-scanner.session.empty') || 'No active sessions'}</p>
				</div>
			{:else}
							...
							<div class="session-info">
								<div class="info-row">
									<span class="info-label"
										>{i18n.t('routes/apps/token-balance-scanner.session.progress') ||
											'Progress'}:</span
									>
								...
								<div class="info-row">
									<span class="info-label"
										>{i18n.t('routes/apps/token-balance-scanner.session.addresses') ||
											'Addresses'}:</span
									>
								...
								<div class="info-row">
									<span class="info-label"
										>{i18n.t('routes/apps/token-balance-scanner.session.tokens') || 'Tokens'}:</span
									>
									<span class="info-value">{session.tokens.length}</span>
								</div>
								<div class="info-row">
									<span class="info-label"
										>{i18n.t('routes/apps/token-balance-scanner.session.last_activity') ||
											'Last Activity'}:</span
									>
								...
								{#if canResume(session)}
									<button class="action-btn resume-btn" onclick={() => onResumeSession(session)}>
										...
										{session.status === 'completed'
											? i18n.t('routes/apps/token-balance-scanner.session.view') || 'View'
											: i18n.t('routes/apps/token-balance-scanner.session.resume') || 'Resume'}
									</button>
								{/if}
								{#if canAbandon(session)}
									<button class="action-btn abandon-btn" onclick={() => handleAbandon(session)}>
										<XCircle size={14} />
										{i18n.t('routes/apps/token-balance-scanner.session.abandon') || 'Abandon'}
									</button>
								{/if}
								{#if canDelete(session)}
									<button class="action-btn delete-btn" onclick={() => handleDelete(session)}>
										<Trash2 size={14} />
										{i18n.t('routes/apps/token-balance-scanner.session.delete') || 'Delete'}
									</button>
								{/if}
							</div>
						</div>
		...
		<div class="modal-footer">
			<button class="new-scan-btn" onclick={onStartNew}>
				<Plus size={16} />
				{i18n.t('routes/apps/token-balance-scanner.session.start_new') || 'Start New Scan'}
			</button>
		</div>
	</div>
</div>
