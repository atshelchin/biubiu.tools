<script lang="ts">
	...
	async function handleFileSelect(event: Event) {
		...
		try {
			...
			if (stored) {
				...
				importError = i18n.t('routes/apps/one-to-many-transfer.step5.session.import_error');
			}
		} catch {
			importError = i18n.t('routes/apps/one-to-many-transfer.step5.session.import_error');
		} finally {
	...
	function formatTimeRemaining(expiresAt: number): string {
		...
		if (remaining <= 0) return i18n.t('routes/apps/one-to-many-transfer.step5.session.expired');
...
<div class="session-manager">
	<div class="header">
		<div class="header-title">
			<History size={20} />
			<h3>{i18n.t('routes/apps/one-to-many-transfer.step5.session.title')}</h3>
		</div>
		<div class="header-actions">
			<button
				class="btn-icon"
				onclick={handleImportClick}
				title={i18n.t('routes/apps/one-to-many-transfer.step5.session.import')}
			>
				<Upload size={18} />
			</button>
			<input
	...
	<div class="session-list">
		{#if sessions.length === 0}
			<div class="empty-state">
				...
				<p>{i18n.t('routes/apps/one-to-many-transfer.step5.session.empty')}</p>
			</div>
		{:else}
					...
					<div class="session-main">
						...
						<div class="session-info">
							<span class="session-recipients">
								...
								{i18n.t('routes/apps/one-to-many-transfer.step5.content.recipients').toLowerCase()}
							</span>
						...
						<div class="session-meta">
							...
							<span class="session-time">
								...
								{:else}
									{i18n.t('routes/apps/one-to-many-transfer.step5.session.expires_in')}
					...
					<div class="session-actions">
						{#if resumable}
							<button class="btn-action btn-primary" onclick={() => handleResume(session)}>
								<Play size={16} />
								{i18n.t('routes/apps/one-to-many-transfer.step5.session.resume')}
							</button>
						{/if}
						<button class="btn-action" onclick={() => handleExport(session.sessionId)}>
							<Download size={16} />
							{i18n.t('routes/apps/one-to-many-transfer.step5.session.export')}
						</button>
						<button class="btn-action btn-danger" onclick={() => handleDelete(session.sessionId)}>
							<Trash2 size={16} />
							{i18n.t('routes/apps/one-to-many-transfer.step5.session.delete')}
						</button>
					</div>
				{/if}
			{/each}
	...
	{#if isImporting}
		<div class="importing-overlay">
			...
			<span>{i18n.t('routes/apps/one-to-many-transfer.step5.session.importing')}</span>
		</div>
	{/if}
</div>
