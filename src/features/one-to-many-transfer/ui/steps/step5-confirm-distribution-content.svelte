<script lang="ts">
	...
	const amountModeLabel = $derived.by(() => {
		switch (step3State.amountMode) {
			case 'equal':
				return i18n.t('routes/apps/one-to-many-transfer.step5.content.mode_equal');
			case 'random':
				return i18n.t('routes/apps/one-to-many-transfer.step5.content.mode_random');
			case 'custom':
				return i18n.t('routes/apps/one-to-many-transfer.step5.content.mode_custom');
			default:
	...
	async function handleExecute() {
		if (!distributionConfig) {
			alert(i18n.t('routes/apps/one-to-many-transfer.step5.content.errors.invalid_config'));
			return;
		}
		...
		if (
			!confirm(
				`${confirmMsg.title}\n\n• ${detailsText}\n\n${confirmMsg.warning}\n\n${i18n.t('routes/apps/one-to-many-transfer.step5.content.confirm_continue')}`
			)
		) {
			return;
		}
...
<StepContent>
	<StepContentHeader
		title={i18n.t('routes/apps/one-to-many-transfer.step5.content.title')}
		description={i18n.t('routes/apps/one-to-many-transfer.step5.content.description')}
	/>
	...
	{#if activeSessions.length > 0}
		<button class="session-toggle" onclick={() => (showSessionManager = !showSessionManager)}>
			<History size={18} />
			<span>{i18n.t('routes/apps/one-to-many-transfer.step5.session.title')}</span>
	...
	<div class="mode-section">
		<h3 class="section-title">
			{i18n.t('routes/apps/one-to-many-transfer.step5.mode.title')}
		</h3>
	...
	{#if connectStore.address && connectStore.currentChainId}
		<div class="wallet-section">
			<h3 class="section-title">
				{i18n.t('routes/apps/one-to-many-transfer.step5.content.executor_wallet')}
			</h3>
			<ExecutorWallet
	...
	<div class="summary-section">
		<h3 class="section-title">
			{i18n.t('routes/apps/one-to-many-transfer.step5.content.summary_title')}
		</h3>
		<div class="summary-grid">
			<div class="summary-card">
				<div class="summary-label">
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.token')}
				</div>
			...
			<div class="summary-card">
				<div class="summary-label">
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.mode')}
				</div>
				<div class="summary-value">
					{amountModeLabel}
				</div>
			...
			<div class="summary-card">
				<div class="summary-label">
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.recipients')}
				</div>
			...
			<div class="summary-card">
				<div class="summary-label">
					{isNFT
						? i18n.t('routes/apps/one-to-many-transfer.step5.content.total_nfts')
						: i18n.t('routes/apps/one-to-many-transfer.step5.content.total_amount')}
				</div>
			...
			<div class="summary-card">
				<div class="summary-label">
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.estimated_gas')}
				</div>
				<div class="summary-value">{estimatedGas} {currentNetwork?.symbol || 'ETH'}</div>
			</div>
			<div class="summary-card">
				<div class="summary-label">
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.network')}
				</div>
	...
	<div class="recipients-section">
		<h3 class="section-title">
			{i18n.t('routes/apps/one-to-many-transfer.step5.content.preview_title')}
		</h3>
		<div class="recipients-preview">
			...
			{#if step4State.totalRecipients > 10}
				<div class="more-recipients">
					...
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.more_recipients')}
				</div>
			{/if}
		</div>
	</div>
	...
	{#if executor.isExecuting || executor.progress.phase !== 'idle'}
		<div class="progress-section" transition:fade>
			<h3 class="section-title">
				{i18n.t('routes/apps/one-to-many-transfer.step5.content.progress_title')}
			</h3>
			...
			{#if executor.summary.success > 0 || executor.summary.failed > 0}
				<div class="results-summary">
					<span class="result-item success">
						...
						{i18n.t('routes/apps/one-to-many-transfer.step5.content.successful')}
					</span>
					{#if executor.summary.failed > 0}
						<span class="result-item failed">
							<X size={14} />
							{executor.summary.failed}
							{i18n.t('routes/apps/one-to-many-transfer.step5.content.failed')}
						</span>
					{/if}
					{#if executor.summary.pending > 0}
						<span class="result-item pending">
							{executor.summary.pending}
							{i18n.t('routes/apps/one-to-many-transfer.step5.content.pending')}
						</span>
					{/if}
				</div>
			{/if}
		...
		<div class="action-section">
			{#if executor.isExecuting}
				<button class="action-button stop" onclick={handleStop}>
					<Pause size={20} />
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.stop')}
				</button>
			{:else if canResume}
				<button class="action-button resume" onclick={handleResume}>
					<RotateCcw size={20} />
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.resume')}
				</button>
				<button class="action-button secondary" onclick={handleReset}>
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.reset')}
				</button>
			{:else if executor.progress.phase === 'completed'}
				<div class="completed-banner">
					<CheckCircle2 size={24} />
					<span>{i18n.t('routes/apps/one-to-many-transfer.step5.content.completed')}</span>
				</div>
				<button class="action-button secondary" onclick={handleReset}>
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.start_new')}
				</button>
			{:else}
				<button class="action-button execute" onclick={handleExecute} disabled={!canExecute}>
					...
					{:else}
						<Play size={20} />
					{/if}
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.execute')}
				</button>
			{/if}
		</div>
	{/if}
	...
	{#if executionMode === 'delegated'}
		...
		{#if delegatedExecutor.session}
			<div class="delegated-section" transition:fade>
				<h3 class="section-title">
					{i18n.t('routes/apps/one-to-many-transfer.step5.delegated.batch_status')}
				</h3>
					...
					{#each delegatedExecutor.session.batches as batch, index (batch.batchId)}
						...
						>
							<div class="batch-info">
								<span class="batch-id">
									{i18n
										.t('routes/apps/one-to-many-transfer.step5.delegated.execute_batch')
								...
								<span class="batch-recipients">
									...
									{i18n
										.t('routes/apps/one-to-many-transfer.step5.content.recipients')
										.toLowerCase()}
								</span>
							</div>
				...
				<div class="delegated-summary">
					<span class="summary-stat">
						...
						{i18n.t('routes/apps/one-to-many-transfer.step5.session.batches')}
					</span>
				</div>
			</div>
		{/if}
			...
			{#if !delegatedExecutor.session}
				...
				<button class="action-button execute" onclick={handleAuthorize} disabled={!canAuthorize}>
					{#if delegatedExecutor.isAuthorizing}
						<Loader2 size={20} class="spinning" />
						{i18n.t('routes/apps/one-to-many-transfer.step5.delegated.authorizing')}
					{:else}
						<Play size={20} />
						{i18n.t('routes/apps/one-to-many-transfer.step5.delegated.authorize')}
					{/if}
				</button>
			...
			{:else}
				...
				>
					...
					{:else}
						<Play size={20} />
					{/if}
					{i18n.t('routes/apps/one-to-many-transfer.step5.delegated.execute_all')}
				</button>
				<button class="action-button secondary" onclick={handleResetDelegated}>
					{i18n.t('routes/apps/one-to-many-transfer.step5.content.reset')}
				</button>
	...
	<div class="warning-box">
		<AlertCircle size={20} />
		<div>
			<strong>{i18n.t('routes/apps/one-to-many-transfer.step5.content.warning_title')}</strong>
			<p>{i18n.t('routes/apps/one-to-many-transfer.step5.content.warning_message')}</p>
		</div>
	</div>
</StepContent>
