...
export async function constructBatchTransaction(
    ...
    const tokenAddresses = tokens
        .filter(t => t.address && t.address !== '0x0' && t.address !== '0x0000000000000000000000000000000000000000')
    ...
    const addresses = privateKeys.map(pk =>
        privateKeyToAccount(pk as Hex).address
    );
...
export async function estimateGas(
    ...
    const publicClient = createPublicClient({
        chain: { id: network.chainId, name: network.name, nativeCurrency: network.nativeCurrency, rpcUrls: { default: { http: [network.rpcUrl] } } },
...
export function formatExecutionResults(results: ExecutionResult[]): string {
    ...
    const totalWallets = results.reduce((sum, r) => sum + r.addresses.length, 0);
    const successfulWallets = results.filter(r => r.success).reduce((sum, r) => sum + r.addresses.length, 0);
    const failedWallets = results.filter(r => !r.success).reduce((sum, r) => sum + r.addresses.length, 0);
