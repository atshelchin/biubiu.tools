	...
	return {
		...
		check(ctx: PaywallContext): PaywallResult {
			...
			if (freeQuota) {
				...
				if (requested <= remaining) {
					return {
						...
						// Legacy fallback
						message: `今日剩余免费额度: ${remaining - requested}`
					};
				}
				...
				return {
					...
					message:
						...
						`今日免费额度已用完 (${freeQuota.daily}/${freeQuota.countBy === 'items' ? '条目' : '次'})`,
					upgradeHint: '升级会员可无限使用'
				};
			}
			...
			return {
				...
				showUpgrade: true,
				// Legacy fallback
				message: options?.message ?? '此功能需要会员'
			};
		}
	};
}
	...
	return {
		...
		check(ctx: PaywallContext): PaywallResult {
			...
			return {
				...
				showPayPerUse: true,
				// Legacy fallback
				message: `本次费用: ${totalFee.toFixed(4)} ETH (${itemCount} × ${perUseFee} ETH)`,
				upgradeHint: '升级会员可免费使用'
			};
		}
	};
}
	...
	return {
		...
		check(ctx: PaywallContext): PaywallResult {
			...
			return {
				...
				showPayPerUse: true,
				// Legacy fallback
				message: `超出免费额度，需付费 ${totalFee.toFixed(4)} ETH`,
				upgradeHint: '升级会员更划算'
			};
		}
	};
