<script lang="ts">
	import { X, PlayCircle, Trash2, Clock, AlertCircle } from '@lucide/svelte';
	...
	let { isOpen = $bindable(), onClose, onRecover }: Props = $props();

	// Get recoverable tasks
	let recoverableTasks = $derived(taskStore.recoverableTasks);
						...
						{#each recoverableTasks as task (task.id)}
							<div class="task-card">
								...
								<div class="task-actions">
									...
									<button class="btn-delete" onclick={() => handleDelete(task)}>
										<Trash2 size={16} />
									</button>
								</div>
							</div>
