...
const MAX_FAIL_COUNT = 3; // Mark as unhealthy after 3 consecutive failures
const HEALTH_RECOVERY_TIME = 60000; // Try unhealthy RPC again after 1 minute

export class RPCManager {
	private endpoints: RPCStatus[];
	private currentIndex: number = 0;

	constructor(rpcEndpoints: RPCEndpoint[]) {
	...
	markFailed(): boolean {
		...
		if (status.failCount >= MAX_FAIL_COUNT) {
			...
			console.log(
				`❌ RPC ${this.getCurrentRPCName()} marked as unhealthy after ${MAX_FAIL_COUNT} failures`
			);
		}
	...
	markRateLimited(): boolean {
		...
		status.isHealthy = false;
		status.failCount = MAX_FAIL_COUNT; // Immediately mark as unhealthy
	...
	private rotateToHealthy(): boolean {
		...
		for (const status of this.endpoints) {
			...
			if (
				!status.isHealthy &&
				status.lastFailure &&
				now - status.lastFailure > HEALTH_RECOVERY_TIME
			) {
				status.isHealthy = true;
				status.failCount = 0;
	...
	getWaitTimeUntilAvailable(): number {
		...
		for (const status of this.endpoints) {
			if (status.rateLimitedUntil && status.rateLimitedUntil > now) {
				...
				const recoveryTime = status.lastFailure + HEALTH_RECOVERY_TIME - now;
				if (recoveryTime > 0) {
