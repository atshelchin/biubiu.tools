...
export function createWalletManager(config: WalletManagerConfig) {
	const {
		projectId,
		appName,
		appUrl,
	...
	// Note: Using unknown as intermediate type to handle viem version mismatch between
	// this project and @shelchin/ethereum-connectors package
	const walletConnectConnector = new WalletConnectConnector({
		chains: chains as unknown as ConstructorParameters<typeof WalletConnectConnector>[0]['chains'],
		shimDisconnect: true,
		projectId,
		showQrModal: false,
		metadata: {
	...
	const coinbaseConnector = new CoinbaseSmartWalletConnector({
		chains: chains as unknown as ConstructorParameters<
			typeof CoinbaseSmartWalletConnector
		>[0]['chains'],
		shimDisconnect: true,
		appName,
		appLogoUrl,
	...
	const injectedConnector = new InjectedConnector({
		chains: chains as unknown as ConstructorParameters<typeof InjectedConnector>[0]['chains'],
