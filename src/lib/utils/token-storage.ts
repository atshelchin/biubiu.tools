...
const STORAGE_KEY = 'custom-tokens';
...
export function addCustomToken(token: ERC20Token): void {
	...
	window.dispatchEvent(
		new CustomEvent('custom-token-added', { detail: { chainId: token.chainId } })
	);
}
...
export function removeCustomToken(tokenId: string): void {
	...
	// Dispatch event to notify other components
	window.dispatchEvent(new CustomEvent('custom-token-removed', { detail: { tokenId } }));
}
