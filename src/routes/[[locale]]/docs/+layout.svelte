<script lang="ts">
	...
	function isActiveDoc(path: string): boolean {
		return $page.url.pathname === `/docs${path}` || $page.url.pathname.endsWith(`/docs${path}`);
	}

	function isActiveCategory(categorySlug: string): boolean {
		return $page.url.pathname.includes(`/docs/${categorySlug}/`);
	}
</script>
					...
					{#if !collapsedCategories.has(category.slug)}
						<ul class="nav-items">
							{#each category.items as item (item.slug)}
								<li>
									<a
										href={localizeHref(`/docs${item.path}`)}
										class="nav-link"
